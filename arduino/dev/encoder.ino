...
#define leftLed 4 // PORTG5
#define rightLed 5 // PORTE3
...
void loop()
{
    // Serial communication is useful for debugging, but it takes a lot of resources
    ...
    if(leftLedState) SET(PORTG,5);
    if(rightLedState) SET(PORTE,3);
    if(!leftLedState) CLR(PORTG,5);
    if(!rightLedState) CLR(PORTE,3);
}
